...
DEFAULT_OUTPUT_DIR = os.getenv("VIDEO_TRANSCRIPT_OUTPUT_DIR", os.path.join(SKILL_DIR, "outputs"))
...
def ytdlp_cmd():
    """Return a yt-dlp command. Prefer PATH executable, fallback to python -m yt_dlp."""
...
def _ytdlp_probe(url):
    ...
    if not ytdlp:
        raise RuntimeError("yt-dlp 未安装")
    cmd = ytdlp + ["--dump-json", "--no-warnings", "--skip-download", url]
